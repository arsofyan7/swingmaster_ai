...
"""
=============================================================================
 SMC Sniper Backtester v3 — Full Pine Translation + Detailed Report
=============================================================================
 Source: smc.pine (Swingmaster : SMC Sniper Visualizer by LuxAlgo)
 
 Translasi PERSIS dari smc.pine ke Python:
   1. Internal structure (size=5) → detect BOS/CHoCH
   2. Swing structure (size=50)   → detect BOS/CHoCH  
   3. Internal Order Blocks stored on structure breaks
   4. OB Mitigation via High/Low (default Pine)
   5. CHoCH state machine (Adventurer section)
   6. BUY signal: activeBullCHoCH + inside bullish OB (POI)
                  + bullish reversal candle + 5-bar cooldown
   7. SL = distal OB, TP = Entry + 2×Risk (RR 2:1)
   8. Position sizing: risk 1% modal per trade
   
 Output: backtest_smc_v3_report.txt
=============================================================================
"""
...
INITIAL_CAPITAL = 10_000_000  # 10 juta
...
class SMCEngine:
    ...
    Pine Script flow:
      1. getCurrentStructure(50) → swing pivots
      2. getCurrentStructure(5, internal=true) → internal pivots  
      3. displayStructure(internal=true) → detect breaks, store OBs
      4. displayStructure() → detect swing breaks
      5. deleteOrderBlocks(internal=true) → mitigate OBs
      6. CHoCH state machine → track active CHoCH
      7. Signal generation → BUY when conditions met
    """
    ...
    def __init__(self, opens, highs, lows, closes, n):
        ...
        # Calculate RMA for ATR (matches TradingView's ta.atr(200))
        alpha = 1.0 / 200
        atr = np.zeros(n)
        atr[0] = self.tr[0]
        for i in range(1, n):
        ...
        # Parsed highs/lows (Pine: high volatility bar filter)
    ...
    def run(self):
        ...
        for i in range(n):
            ...
            # ══════════════════════════════════════════════════════════════
            # 1) STRUCTURE DETECTION: getCurrentStructure()
            # ══════════════════════════════════════════════════════════════
            ...
            # ══════════════════════════════════════════════════════════════
            # 2) STRUCTURE BREAKS: displayStructure()
            # ══════════════════════════════════════════════════════════════
            ...
            #    Pine default: mitigationSource = High/Low
            # ══════════════════════════════════════════════════════════════
            ...
            # ══════════════════════════════════════════════════════════════
            # 4) CHoCH STATE MACHINE (Adventurer section from Pine)
            # ══════════════════════════════════════════════════════════════
            ...
            # ══════════════════════════════════════════════════════════════
            # 5) BUY SIGNAL CHECK (Pine: Adventurer trigger)
            # ══════════════════════════════════════════════════════════════
            ...
            # Bullish reversal candle (Pine: close > open and close > close[1])
            if c <= opens[i]:
                continue
            if c <= pc:
                continue
...
def resolve_trades(signals, highs, lows, closes, dates, n, ticker):
    ...
    One position at a time per ticker. Compounding capital.
    """
    trades: List[Trade] = []
    ...
    for sig in signals:
        ...
        for j in range(entry_idx + 1, n):
            ...
            if lows[j] <= sl_price:
                ...
                trade.result = "LOSS"
                trade.holding_days = j - entry_idx
                current_exit_idx = j
                exit_found = True
                break
        ...
        if not exit_found:
            ...
            trade.result = "OPEN"
            trade.holding_days = (n - 1) - entry_idx
...
def compute_stats(trades: List[Trade], final_capital: float = None) -> dict:
    ...
    for t in trades:
        if t.result == "WIN":
            ...
            cur_con_l += 1
            cur_con_w = 0
        max_con_w = max(max_con_w, cur_con_w)
        max_con_l = max(max_con_l, cur_con_l)
...
def fmt_rp(amount: float) -> str:
    """Format rupiah amount to readable string."""
    if abs(amount) >= 1e12:
...
def generate_report(all_trades: List[Trade], ticker_results: Dict, output_path: str):
    ...
    L.append("  ╚═════╝ ╚═╝     ╚═╝ ╚═════╝    ╚═════╝ ╚═╝  ╚═╝ ╚═════╝╚═╝  ╚═╝   ╚═╝   ╚══════╝╚══════╝   ╚═╝   ")
    L.append("=" * 110)
    L.append(f"  Swingmaster AI — SMC Sniper Backtester v3")
    L.append(f"  Generated     : {now}")
    L.append(f"  Source        : smc.pine (LuxAlgo SMC + Adventurer Signals)")
    ...
    L.append(f"  Cooldown      : {BUY_COOLDOWN_BARS} bars between signals")
    L.append("=" * 110)

    # ════════════════════════════════════════════════════════════════════════════
    # RESUME KESELURUHAN (ALL TICKERS)
    # ════════════════════════════════════════════════════════════════════════════
    ...
    for t in sorted(all_trades, key=lambda x: x.entry_date):
        ...
        if equity > peak:
            peak = equity
        dd = (peak - equity) / peak * 100 if peak > 0 else 0
        max_dd_global = max(max_dd_global, dd)

    # Count profitable tickers
    profitable_tickers = sum(1 for _, (_, s) in ticker_results.items() if s['total'] > 0 and s['return_pct'] > 0)
    losing_tickers = sum(1 for _, (_, s) in ticker_results.items() if s['total'] > 0 and s['return_pct'] < 0)
    total_tickers_traded = sum(1 for _, (_, s) in ticker_results.items() if s['total'] > 0)

    L.append("")
    L.append("┌──────────────────────────────────────────────────────────────────────────────┐")
    L.append("│               📊 RESUME KESELURUHAN (ALL TICKERS)                           │")
    L.append("│               * Setiap ticker dimulai dari modal Rp 10 Juta                 │")
    L.append("├──────────────────────────────────────────────────────────────────────────────┤")
    L.append(f"│  Total Tickers Traded   : {total_tickers_traded:>48d}  │")
    L.append(f"│  Tickers Profitable     : {profitable_tickers:>48d}  │")
    L.append(f"│  Tickers Rugi           : {losing_tickers:>48d}  │")
    L.append(f"│  Total Trades           : {overall_total:>48d}  │")
    L.append(f"│  Wins                   : {overall_wins:>48d}  │")
    L.append(f"│  Losses                 : {overall_losses:>48d}  │")
    L.append(f"│  Open (belum close)     : {overall_open:>48d}  │")
    L.append(f"│  Win Rate               : {overall_wr:>47.2f}%  │")
    L.append(f"│  Avg Win  (% capital)   : {avg_win:>47.2f}%  │")
    L.append(f"│  Avg Loss (% capital)   : {avg_loss:>47.2f}%  │")
    L.append(f"│  Max Single Win         : {max_win:>47.2f}%  │")
    L.append(f"│  Max Single Loss        : {max_loss:>47.2f}%  │")
    L.append(f"│  Profit Factor          : {pf:>48.2f}  │")
    L.append(f"│  Avg Holding Days       : {avg_holding:>47.1f}d  │")
    L.append("└──────────────────────────────────────────────────────────────────────────────┘")

    # ════════════════════════════════════════════════════════════════════════════
    # TABLE: PER TICKER SUMMARY (sorted by Return)
    # ════════════════════════════════════════════════════════════════════════════
    L.append("")
    L.append("=" * 110)
    ...
    header = (
        ...
        f"{'WR%':>7}│{'AvgW%':>7}│{'AvgL%':>7}│"
        f"{'Return%':>10}│{'Modal Akhir':>18}│{'PF':>7}│{'MaxDD%':>7}│{'AvgHold':>8}"
    )
    L.append(header)
    L.append("─" * len(header))
    ...
    for tk in sorted_tickers:
        ...
        line = (
            ...
            f"{stats['avg_win_pct']:>6.2f}%│{stats['avg_loss_pct']:>6.2f}%│"
            f"{stats['return_pct']:>9.2f}%│"
            f"{fmt_rp(stats['final_capital']):>18s}│{pf_s:>7s}│"
            f"{stats['max_drawdown_pct']:>6.1f}%│"
            f"{stats['avg_holding_days']:>6.1f}d"
        )
        L.append(line)
    ...
    best_tickers = [tk for tk in sorted_tickers if ticker_results[tk][1]['total'] >= 3][:20]
    for i, tk in enumerate(best_tickers):
        trades, s = ticker_results[tk]
        L.append(
    ...
    worst_tickers = [tk for tk in reversed(sorted_tickers) if ticker_results[tk][1]['total'] >= 3][:20]
    for i, tk in enumerate(worst_tickers):
        trades, s = ticker_results[tk]
        L.append(
            f"  {i+1:>2}. {tk:<10}│ Return: {s['return_pct']:>8.2f}% │ "
            f"Win Rate: {s['win_rate']:>5.1f}% │ Trades: {s['total']:>4} │ "
    ...
    # ════════════════════════════════════════════════════════════════════════════
    # DETAIL PER TICKER: Last 10 triggers
    # ════════════════════════════════════════════════════════════════════════════
    L.append("")
    L.append("=" * 110)
    L.append("  📋 DETAIL PER TICKER — Last 10 Triggers")
    L.append("=" * 110)
    ...
    for tk in sorted_tickers:
        ...
        L.append(f"  ┌─── {tk} ───────────────────────────────────────────────────────────────────")
        L.append(f"  │  Modal Awal   : {fmt_rp(INITIAL_CAPITAL)}")
        L.append(f"  │  Modal Akhir  : {fmt_rp(stats['final_capital'])}")
        L.append(f"  │  Return       : {stats['return_pct']:+.2f}%")
        L.append(f"  │  Win Rate     : {stats['win_rate']:.1f}% ({stats['wins']}W / {stats['losses']}L / {stats['open']}O dari {stats['total']} trades)")
        L.append(f"  │  Profit Factor: {stats['profit_factor']:.2f}")
        L.append(f"  │  Max Drawdown : {stats['max_drawdown_pct']:.1f}%")
        L.append(f"  │  Max Consec W : {stats['max_consecutive_wins']}  │  Max Consec L: {stats['max_consecutive_losses']}")
        L.append(f"  │  Avg Hold Days: {stats['avg_holding_days']:.1f}")
        L.append(f"  │")

        # Last 10 trades detail
        last_n = trades[-10:]
        if last_n:
            L.append(f"  │  Last {len(last_n)} Triggers:")
            L.append(f"  │  {'No':>4} │ {'Entry Date':<12} │ {'Entry':>10} │ {'SL':>10} │ {'TP':>10} │ {'Exit Date':<12} │ {'Exit':>10} │ {'P&L':>14} │ {'Result':<6} │ {'Hold':>5}")
            L.append(f"  │  {'─'*4}─┼─{'─'*12}─┼─{'─'*10}─┼─{'─'*10}─┼─{'─'*10}─┼─{'─'*12}─┼─{'─'*10}─┼─{'─'*14}─┼─{'─'*6}─┼─{'─'*5}")

            for idx, t in enumerate(last_n):
                entry_d = t.entry_date[:10] if len(t.entry_date) > 10 else t.entry_date
                exit_d = t.exit_date[:10] if len(t.exit_date) > 10 else t.exit_date
                result_icon = "✅" if t.result == "WIN" else "❌" if t.result == "LOSS" else "⏳"
                pnl_str = fmt_rp(t.pnl)
                L.append(
                    f"  │  {idx+1:>4} │ {entry_d:<12} │ {t.entry_price:>10,.0f} │ {t.sl_price:>10,.0f} │ "
                    f"{t.tp_price:>10,.0f} │ {exit_d:<12} │ {t.exit_price:>10,.0f} │ "
                    f"{pnl_str:>14} │ {result_icon} {t.result:<4} │ {t.holding_days:>4}d"
                )
...
def main():
    ...
    output_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'backtest_smc_v3_report.txt')
    ...
    print("=" * 60)
    print("  SMC Sniper Backtester v3 — Full Pine Translation")
    print(f"  Modal: {fmt_rp(INITIAL_CAPITAL)} │ Risk: {RISK_PCT*100:.0f}% │ RR: 1:{int(RR_RATIO)} │ Buy Only")
    print("=" * 60)
    ...
    for ticker in tickers:
        ...
        if processed % 50 == 0:
            ...
            eta = (elapsed / processed) * (len(tickers) - processed - skipped)
            print(f"      {processed}/{len(tickers)} tickers... "
                  f"({len(all_trades)} trades, ETA: {eta:.0f}s)")

    elapsed = (datetime.now() - start_time).total_seconds()
    print(f"\n      Selesai! {processed} diproses, {skipped} diskip ({elapsed:.1f}s)")
    ...
    profitable_count = sum(1 for _, (_, s) in ticker_results.items() if s['total'] > 0 and s['return_pct'] > 0)
    losing_count = sum(1 for _, (_, s) in ticker_results.items() if s['total'] > 0 and s['return_pct'] < 0)

    print()
    print("┌──────────────────────────── QUICK SUMMARY ────────────────────────────┐")
    print(f"│  Total Trades       : {total:>45}  │")
    print(f"│  Win / Loss         : {wins:>20} / {losses:<22}  │")
    print(f"│  Win Rate           : {wr:>44.2f}%  │")
    print(f"│  Tickers Profitable : {profitable_count:>45}  │")
    print(f"│  Tickers Rugi       : {losing_count:>45}  │")
    print(f"│  Modal per Ticker   : {fmt_rp(INITIAL_CAPITAL):>45}  │")
    print(f"│  Risk per Trade     : {RISK_PCT*100:>43.0f}%  │")
    print(f"│  RR Ratio           : {'1:' + str(int(RR_RATIO)):>45}  │")
    print("└───────────────────────────────────────────────────────────────────────┘")
